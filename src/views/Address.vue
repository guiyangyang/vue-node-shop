        ...
        <div class="container">
          <div class="checkout-addr">
            ...
            <div class="addr-list-wrap">
              <div class="addr-list">
                <ul>
                  <li v-for="(item,index) in addressListFliter" v-bind:class="{'check':checkIndex == index}" @click="checkIndex = index;selectedAddressId=item.addressId">
                    <dl>
                      <dt>{{item.userName}}</dt>
                    ...
                    <div class="addr-opration addr-set-default">
                      <a href="javascript:;" class="addr-set-default-btn"  v-if="!item.isDefault" @click="setDefualt(item.addressId)"><i>Set default</i></a>
                    </div>
...
<script>
  ...
  data (){
    return {
       limit:3,//默认 显示 几条 地址
       addressList:[],
       selectedAddressId:'', //选中 地址的id 用于 结算
       checkIndex:0,   //选中地址
       isMdShow:false,
       addressId:'' //删除 地址的id 用于 结算
    }
  },
  mounted (){
    this.init()
  ...
  computed:{
    addressListFliter (){//默认 显示 几条 地址
      return this.addressList.slice(0,this.limit);
      //数组 操作arr.slice(start,stop) 片段 arr.splice(a,b,item) 剪接
    }
  },
  components:{
    NavHeader,
  ...
  methods:{
    ...
    setDefualt(addressId){
      axios.post('/users/setDefualt',{
        addressId:addressId
      }).then((response)=>{
        let res = response.data;
        if(res.status == 0){
