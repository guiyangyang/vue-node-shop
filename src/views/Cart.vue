...
import {currency} from './../util/currency' // 金额 格式化 过滤器
// export name = 需要 用{} 导入接受
...
export default{
  ...
  filters:{  // 局部 过滤器 实现 若是 全局 过滤器 则在 main.js 中定义
    currency:currency
  },
  components:{
    NavHeader,
  ...
  computed:{
     //计算 属性  data中就不用写了 //商品 选中 数量 是否 等于 总数
     //计算属性  一直在 计算，别处 是无法直接赋值
    checkAllFlag (){
  ...
  methods:{
    ...
    delCart (){
      // console.log("delCart:"+this.productId)
    ...
    checkOut (){
      if(this.checkedCount > 0){ // 当选中商品时 可跳转结算页面
        this.$router.push({  //代码  控制 视图 路由
          path:"/address"
        })
