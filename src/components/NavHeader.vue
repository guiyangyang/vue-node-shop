    ...
    <div class="navbar">
      ...
      <div class="navbar-right-container" style="display: flex;">
        <div class="navbar-menu-container">
          ...
          <span class="navbar-link" v-text="niceName" v-if="niceName"></span>
          <a href="javascript:void(0)" class="navbar-link" @click="loginModalFlag=true" v-if="!niceName">Login</a>
          <a href="javascript:void(0)" class="navbar-link" v-if="niceName" @click="logOut">Logout</a>
...
export default {
  ...
  mounted (){
   this.checkLogin();// 检验 是否已登录
  },
  computed:{
    //1. 方法一
    // niceName (){// 2. 用 vuex管理  用户名
    //   return this.$store.state.niceName;
    // },
    // cartCount (){
    //   return this.$store.state.cartCount;
    // }
    // 2. 方法二
  ...mapState(['niceName','cartCount'])
  ...
  methods:{
    ...
    axios.get('/users/checkLogin').then((response)=>{
      ...
      if(res.status==0){
        // this.niceName =res.result;//3. 现在 改用 vuex
    ...
    login (){
      ...
      // this.niceName = this.userName
         axios.post("/users/login",{
           ...
           let res = response.data;
           console.log('login 001')
           console.log(res)
           if(res.status =='0'){
             this.errorTip=false;
             this.loginModalFlag = false
             // this.niceName = res.result.userName // 改用 vuex
    ...
    getCartCount (){// 4. vuex 管理购物车 商品数量 显示
